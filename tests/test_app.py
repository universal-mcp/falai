...
@pytest.fixture
def app_instance():
    """Provides a FalaiApp instance for tests."""
...
def test_universal_mcp_falai_app_initialization(app_instance):
    """
    Test that the FalaiApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "falai", "FalaiApp instance has unexpected name."


def test_universal_mcp_falai_tool_docstrings_format(app_instance):
    """
    Test that each tool method in FalaiApp has a well-formatted docstring,
...
def test_universal_mcp_falai_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in FalaiApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
